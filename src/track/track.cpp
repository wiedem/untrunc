...
void Track::parseOk() {
	...
	if (!header_atom_) throw std::runtime_error("No mdhd atom: unknown duration and timescale");
	readHeaderAtom();
	...
	if (name_size > 0) {
		...
		// special case: pascal string (first byte is the length)
		if (!handler_name_.empty() && (unsigned char)handler_name_[0] == (unsigned)(name_size - 1))
			handler_name_.erase(0, 1);
		trim_right(handler_name_);
	}

	if (handler_type_ != "soun" && handler_type_ != "vide" && !g_options.show_tracks) {
		logg(I, "special track found (", handler_type_, ", '", handler_name_, "')\n");
	}
	do_stretch_ = g_options.stretch_video && handler_type_ == "vide";
	...
	if (codec_.name_ == "sowt" || codec_.name_ == "twos") {
		...
		if (constant_size_ != expected_size) {
			logg(W2, "using expected ", codec_.name_, " frame size of ", nc, "*", expected_size, ", instead of ",
			     constant_size_, " as found in stsz\n");
...
void Track::writeToAtoms(bool broken_is_64) {
	...
	if (broken_is_64 && stbl->atomByName("stco")) {
		stbl->prune("stco");
		auto new_co64 = std::make_unique<Atom>();
		new_co64->name_ = "co64";
		stbl->children_.push_back(std::move(new_co64));
	} else if (!broken_is_64 && stbl->atomByName("co64")) {
		stbl->prune("co64");
		auto new_stco = std::make_unique<Atom>();
		new_stco->name_ = "stco";
		stbl->children_.push_back(std::move(new_stco));
	}
	...
	//#define SHANE 6
#ifdef SHANE
	int pps[15] = {0x28ee3880, 0x28ee1620, 0x28ee1e20, 0x28ee0988, 0x28ee0b88, 0x28ee0d88, 0x28ee0f88, 0x28ee0462,
	               0x28ee04e2, 0x28ee0562, 0x28ee05e2, 0x28ee0662, 0x28ee06e2, 0x28ee0762, 0x28ee07e2};
	if (codec.name == "avc1") {
...
void Track::clear() {
	if (orig_sizes_.empty())
		swap(sizes_, orig_sizes_);
	else
		sizes_.clear();

	if (orig_times_.empty())
		swap(times_, orig_times_);
	else
		times_.clear();
...
void Track::calcAvgSampleSize() {
	...
	int k = keyframes_.size() ? keyframes_[0] : -1, ik = 0;
	for (int i = 0; i < (int)sizes_.size(); i++) {
		int sz = sizes_[i];
		bool is_keyframe = k == i;

		if (is_keyframe && ++ik < (int)keyframes_.size()) k = keyframes_[ik];
...
void Track::saveSampleTimes() {
	...
	if (constant_duration_ != -1) {
		...
		for (uint i = 0; i < times_.size(); i++) {
			...
			if (!vp.empty() && v == vp.back().second)
				vp.back().first++; // don't repeat same value
			else
				vp.emplace_back(1, v);
		}
	}
...
void Track::saveKeyframes() {
	...
	Atom *stss = trak_->atomByName("stss");
	if (!stss) {
		auto new_stss = std::make_unique<Atom>();
		new_stss->name_ = "stss";
		stss = new_stss.get();
		trak_->children_.push_back(std::move(new_stss));
	}
...
void Track::saveCompositionOffsets() { // aka DisplayOffsets
	...
	if (!orig_ctts_.size() || g_options.no_ctts) {
		if (g_options.no_ctts) logg(I, "pruning ctts..\n");
		trak_->prune("ctts");
		return;
	}
...
bool Track::shouldUseChunkPrediction() {
	if (!hasPredictableChunks()) return false;
	if (!isSupported()) return true;
	// When dynamic chunk stats are enabled, codecs that cannot report frame size
	// via the decoder (e.g. mp4a on FFmpeg 8.x) have frame-level detection
	// disabled. Chunk prediction is the only recovery path, so always allow it.
	if (g_options.use_chunk_stats && codec_.needsDynStatsForSizing()) return true;
	return likely_samples_sizes_p >= 0.99;
}
...
int64_t Track::stepToNextOwnChunk(off_t off) {
	...
	logg(V, "stepToNextOwnChunkOff(", off, "): to: ", codec_.name_, " last chunk_off: ", current_chunk_.off_,
	     " next: ", off + step, "\n");
	return step;
}
...
int64_t Track::stepToNextOtherChunk(off_t off) {
	...
	if (!is_dummy_) { // jpeg
		...
		logg(V, "stepToNextOtherChunkOff(", off, "): from: ", codec_.name_, ", step: ", step, ", next: ", off + step,
		     "\n");
		return step;
	}
	return step;
...
bool Track::chunkProbablyAtAnd() {
	...
	for (auto n : likely_n_samples_)
		if (current_chunk_.n_samples_ == n) return true;
	return false;
}
...
void Track::splitChunks() {
	...
	for (uint i = 0; i < orig_chunks.size(); i++) {
		...
		auto off = c.off_;
		for (uint j = 0; j < (uint)c.n_samples_; j++) {
			auto sz = getSize(pkt_idx++);
			chunks_.emplace_back(Track::Chunk(off, sz, 1));
			if (g_options.dont_exclude) sz = alignPktLength(sz);
			off += sz;
		}
	}
}
...
void Track::genChunkSizes() {
	if (!chunks_.size())
		throw logic_error(
		    ss("healthy file has a '", codec_.name_, "' track, but no single ", codec_.name_, "-frame!\n"));
