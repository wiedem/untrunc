...
void FileRead::open(const string &filename) {
	...
	if (!file_) throw std::runtime_error("Could not open file '" + filename + "': " + strerror(errno));

	fseeko(file_, 0L, SEEK_END);
	size_ = ftello(file_);
	fseeko(file_, 0L, SEEK_SET);

	if (!isRegularFile(fileno(file_))) throw std::runtime_error("not a regular file: " + filename);

	buffer_ = std::make_unique<uchar[]>(buf_size_);
	fread(buffer_.get(), 1, buf_size_, file_);
}
...
size_t FileRead::fillBuffer(off_t location) {
	...
	if (avail < 0 || avail >= buf_size_) {
		...
		int n = fread(buffer_.get(), 1, buf_size_, file_);
		return n;
	} else if (avail > 0) {
		memmove(buffer_.get(), buffer_.get() + buf_loc, buf_size_ - buf_loc);
	}
	int n = fread(buffer_.get() + avail, 1, buf_size_ - avail, file_);
	return n;
}
...
size_t FileRead::readBuffer(uchar *dest, size_t size, size_t n) {
	...
	if (avail < total) {
		...
		memcpy(dest, buffer_.get() + buf_off_, avail);
		nread = avail;
		total -= avail;
		buf_off_ = buf_size_;
		if (total >= to_uint(buf_size_)) {
			...
			memcpy(dest + nread, buffer_.get(), x);
			buf_off_ += x;
			nread += x;
		}
	} else {
		memcpy(dest, buffer_.get() + buf_off_, total);
		buf_off_ += total;
		nread = total;
	}
	return nread / size;
...
uint FileRead::readInt() {
	...
	if (n != 1) throw std::runtime_error("Could not read integer");
	return swap32(value);
}
...
int64_t FileRead::readInt64() {
	...
	if (n != 1) throw std::runtime_error("Could not read int64");
...
void FileRead::readChar(char *dest, size_t n) {
	...
	if (len != n) {
		...
		throw std::runtime_error("Could not read chars");
	}
}
...
vector<uchar> FileRead::read(size_t n) {
	...
	if (len != n) throw std::runtime_error("Could not read at position");
	return dest;
}
...
FileWrite::FileWrite(const string &filename) {
	...
	if (!file_) throw std::runtime_error("Could not create file '" + filename + "': " + strerror(errno));
}
