...
Atom::Atom(const Atom &other)
    : start_(other.start_), length_(other.length_), header_length_(other.header_length_), name_(other.name_),
      content_(other.content_), cursor_off_(other.cursor_off_) {
	for (const auto &child : other.children_)
		children_.push_back(std::make_unique<Atom>(*child));
}
...
void Atom::parseHeader(FileRead &file, bool no_check) {
	...
	if (start_ < 0) throw std::runtime_error(ss("atom start: ", start_)); //  this is impossible?
	for (int i = 0; i < 4; i++)
		if (!isalnum(name_[i]) && !isspace(name_[i])) throw std::runtime_error(ss("invalid atom name: '", name_, "'"));
}
...
void Atom::parse(FileRead &file) {
	...
	if (isParent(name_) && name_ != "udta") { //user data atom is dangerous... i should actually skip all
		...
		if (!isPointingAtAtom(file)) logg(W, "bad 'mdat' length = ", length_, " new_pos = ", file.pos(), "\n");
	} else {
		content_ = file.read(length_ - 8); //lenght includes header
		if (content_.size() < to_uint(length_ - 8))
			throw std::runtime_error(ss("Failed reading atom content: ", name_));
...
off_t Atom::findNextAtomOff(FileRead &file, const Atom *start_atom, bool searching_rootlvl) {
	...
	bool skip_nested =
	    start_atom->length_ > 0 &&
	    ((searching_rootlvl && start_atom->name_ != "avcC" && next_off < file.length()) || start_atom->name_ == "mdat");
	...
	if (searching_rootlvl && !--show_bad_msg_in) logg(I, "'", file.filename_, "' has invalid atom lengths, see '-f'\n");

	off_t off_start = start_atom->contentStart(), off = off_start;
	while (off < file.length()) {
		auto buff = file.getPtrAt(off + 4, 7);
		if (g_options.log_mode == LogMode::I && off % (1 << 16) == 0 && off > off_start)
			outProgress(off, file.length());

		const char m = *(buff + 3);
		if (!isdigit(m) && !islower(m) && !isspace(m)) {
			off += 4;
			continue;
		}
...
void Atom::print(int offset) {
	...
	if (g_atom_names.count(name_)) cout << " \"" << g_atom_names.at(name_) << "\"";
	cout << " [" << start_ << ", " << length_ << "]\n";
	...
	} else if (name_ == "stsc") { //samples to chunk:
		//lets just read the first entry
		int entries = readInt(4);
		cout << indent << " Entries: " << entries << endl;
		for (int i = 0; i < entries && i < 10; i++)
			cout << indent << " chunk: " << readInt(8 + 12 * i) << " nsamples: " << readInt(12 + 12 * i)
...
vector<Atom *> Atom::atomsByName(const string &name, bool no_recursive) {
	...
	for (auto &child : children_) {
		if (child->name_ == name) atoms.push_back(child.get());

		if (!no_recursive) {
			vector<Atom *> a = child->atomsByName(name);
...
Atom *Atom::atomByNameSafe(const string &name) {
	...
	if (!atom) throw std::runtime_error(ss("Missing atom: '", name, "' (= ", getAtomNameFromCode(name), ")"));
	return atom;
}

void Atom::replace(Atom *original, Atom *replacement) {
	for (auto &child : children_) {
		if (child.get() == original) {
...
void Atom::prune(Atom *child) {
	auto idx = std::find_if(children_.begin(), children_.end(), [child](const auto &p) { return p.get() == child; });
...
const uchar *BufferedAtom::getFragment(off_t offset, int size) {
	if (offset < 0) throw out_of_range(ss("Offset set before beginning of mdat (", offset, ")"));
	if (offset + size > contentSize())
		throw out_of_range(
		    ss("Out of Range: ", offset + size, " / ", contentSize(), " (+", offset + size - contentSize(), ")"));
...
void BufferedAtom::write(FileWrite &output, bool force_64) {
	...
	while (offset < contentSize()) {
		if (name_ == "mdat" && g_options.log_mode == I && loop_cnt++ >= 10) {
		...
		if (!at_end(to_skip_it) && offset == to_skip_it->first) {
			logg(V, "skipping ", setfill(' '), left, setw(6), to_skip_it->second, " at ", hexIf(to_skip_it->first),
			     '\n');
