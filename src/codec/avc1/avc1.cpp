...
int getSizeAvc1(Codec *self, const uchar *start, uint maxlength) {
	...
	if (!sps_info_initialized) {
		logg(V, "sps_info (before): ", sps_info.frame_mbs_only_flag, ' ', sps_info.log2_max_frame_num, ' ',
	...
	while (1) {
		...
		case NAL_SPS:
			...
			if (!sps_info.is_ok) sps_info.decode(nal_info.data_);
			break;
		case NAL_AUD: // Access unit delimiter
			if (!previous_slice.is_ok) break;
			return length;
		case NAL_IDR_SLICE:
			was_keyframe = true;
			[[fallthrough]];
		case NAL_SLICE: {
			...
			if (!previous_slice.is_ok) {
				...
				if (slice_info.isInNewFrame(previous_slice)) return length;
		...
		case NAL_FILLER_DATA:
			...
			if (g_options.skip_nal_filler_data) return length;
			break;
		default:
			if (previous_slice.is_ok) {
		...
		if (self->ss_stats_->wouldExceed("avc1", length, nal_info.length_, self->was_keyframe_)) {
			return length;
		}
