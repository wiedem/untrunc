...
static LenResult getLengths(Codec *self, const uchar *start, uint maxlength) {
	LenResult r;
	int &length = r.length;
	...
	while (1) {
		logg(V, "---\n");
		logg(V, "pos: ", self->mp4_->offToStr(self->cur_off_ + length), "\n");
		...
		if (self->ss_stats_->wouldExceed("hvc1", length, nal_info.length_,
		                                 g_options.allow_large_sample ? 1 : self->was_keyframe_)) {
			return r;
		}
