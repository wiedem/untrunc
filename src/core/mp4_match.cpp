...
bool Mp4::shouldBeStrict(off_t off, int track_idx) {
	...
	if (!ctx_.scan_.unknown_length_ || !t.isSupported()) return false;
	if (!g_options.use_chunk_stats || !tracks_.back().is_dummy_ || !t.chunkProbablyAtAnd() || !t.isChunkOffsetOk(off))
		return true;

	assertt(ctx_.scan_.last_track_idx_ == idx_free_);
...
bool Mp4::wouldMatch(const WouldMatchCfg &cfg) {
	...
	for (uint i = 0; i < tracks_.size(); i++) {
		...
		if (cfg.very_first && ctx_.scan_.orig_first_track_->codec_.name_ != c.name_) continue;
	...
	if (g_options.use_chunk_stats) {
		auto last_idx = cfg.last_track_idx >= 0 ? cfg.last_track_idx : ctx_.scan_.last_track_idx_;
		if (cfg.very_first) { // very first offset
			assertt(ctx_.scan_.last_track_idx_ == -1);
			bool r = anyPatternMatchesHalf(cfg.offset, getTrackIdx(ctx_.scan_.orig_first_track_->codec_.name_));
			logg(V, "wouldMatch(", cfg, ") -> ", r, "  // via anyPatternMatchesHalf(",
			     ctx_.scan_.orig_first_track_->codec_.name_, ")\n");
			return r;
		}
...
FrameInfo Mp4::predictSize(const uchar *start, int track_idx, off_t offset) {
	...
	if (track.constant_size_ > 0) {
		...
		int length_signed = c.getSize(start, ctx_.file_.current_maxlength_, offset);
		...
		if (length > ctx_.file_.current_maxlength_) {
			logg(V, "limit: ", min(ctx_.file_.max_part_size_, ctx_.file_.current_maxlength_), "\n");
			logg(E, "Invalid length: ", length, " - too big (track: ", track_idx, ")\n");
			return FrameInfo();
		}
		if (length < kMinAvc1FrameLen && c.name_ == "avc1") { // very short NALs are ok, short frames aren't
			logg(W2, "Invalid length: ", length, " - too small (track: ", track_idx, ")\n");
			return FrameInfo();
		}
		if (ctx_.scan_.unknown_length_ && track.ss_stats_.likelyTooSmall(length)) {
			return FrameInfo();
		}
	}
...
FrameInfo Mp4::getMatch(off_t offset, bool force_strict) {
	...
	// hardcoded match
	if (ctx_.scan_.pkt_idx_ == 4 && hasCodec("tmcd") && getTrack("tmcd").is_tmcd_hardcoded_) {
		if (!wouldMatch(WMCfg{offset, "", true, ctx_.scan_.last_track_idx_})) {
	...
	for (uint i = 0; i < tracks_.size(); i++) {
		...
		Codec &c = track.codec_;
		logg(V, "Track codec: ", c.name_, '\n');
		if (g_options.use_chunk_stats && (to_uint(ctx_.scan_.last_track_idx_) != i || ctx_.scan_.unknown_length_) &&
...
bool Mp4::anyPatternMatchesHalf(off_t offset, uint track_idx_to_try) {
	auto buff = ctx_.file_.mdat_->getFragment(offset, Mp4::pat_size_ / 2);
