...
void Mp4::repairRsvBen(const string &filename) {
	...
	// Set up ctx_.current_mdat_ for the RSV file as if it were an mdat starting at offset 0
	ctx_.current_mdat_ = std::make_unique<BufferedAtom>(file_read);
	ctx_.current_mdat_->name_ = "mdat";
	ctx_.current_mdat_->start_ = -8; // Pretend mdat header is at -8 so content starts at 0
	ctx_.current_mdat_->file_end_ = file_read.length();
	...
	while (pos < file_size) {
		...
		if (audio_track_idx >= 0 && audio_start + total_audio_chunk_size <= file_size) {
			...
			for (int t = 0; t < num_audio_tracks; t++) {
				...
				ctx_.pkt_idx_ += samples_in_chunk;
			}
